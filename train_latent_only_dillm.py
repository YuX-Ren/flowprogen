...
rmtree('./results/train_latent_only_dillm', ignore_errors = True)
results_folder = Path('./results/train_latent_only_dillm')
...
model = LLMfusion(
    ...
    add_pos_emb = True,
    modality_num_dim = 2,
    velocity_consistency_loss_weight = 0.01,  # 降低权重，提高稳定性
    reconstruction_loss_weight = 0.01,        # 降低权重，提高稳定性
    transformer={
        'use_llama': True,  # 指定使用 Llama
        'dim': 2048,
        'model_name_or_path': '/share/project/xiaohongwang/LLM_checkpoints/Llama3.2/Llama-3.2-1B-Instruct',
        # 'torch_dtype': torch.float32,  # 使用 float32 而不是 float16
...
# 创建 EMA 模型
ema_model = model.create_ema(0.999)  # 使用更大的 EMA 衰减率，提高稳定性
...
class FlowersDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (tensor / 255.).to(torch.float32)  # 返回 float32 类型的数据
...
for step in range(1, 100_000 + 1):
    ...
    try:
        for _ in range(2):  # 进一步减少累积步数，提高稳定性
