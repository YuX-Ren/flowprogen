...
def transflow_attn_mask(modalities: Int['b m 3']):
    ...
    def forward(
        ...
        if needs_masking:
            ...
            if exists(modality_positions):
                ...
                if should_use_flex_attn:
                    transflow_mask_fn = transflow_attn_mask(modality_positions)
                    block_mask = create_block_mask(transflow_mask_fn, B = None, H = None, Q_LEN = seq_len, KV_LEN = seq_len, _compile = True, device = device)
                    attn_mask_kwargs.update(block_mask = block_mask)
                else:
